...
namespace LoBridge
{
    public partial class BridgeOut : Form
    {
        ...
        private void BridgeOut_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dt = dq.GetDataTable("SELECT Lading_Out.ladingID, Lading_Out.Qty, Lading_Out.Date, Lading_Out.LOusername, Lading_Out.Loading_Station, Lading_Out.F_weight, Lading_Out.S_weight, Lading_Out.Net_Weight, Commodity.Comm_NAME, "
        ...
        private void button1saveFW_Click(object sender, EventArgs e)
        {
            try
            {
                string upquery = "Update lading_out set F_weight=" + f_weightLabel1.Text + ", F_Wdate=CURRENT_TIMESTAMP, Status=N'ثان'" +
                    ", F_weigherName=N'" + Properties.Settings.Default.logedUN + "', Weight_Notes=N'" + weight_NotesTextBox.Text + "' where ladingid=" + ladnum;
            ...
            if (Valid())
            {
                try
                {
                    string upquery = "Update lading_out set S_weight=" + s_weightLabel1.Text + ", S_Wdate=CURRENT_TIMESTAMP, Status=N'تم'" +
                        ",Loading_Station=N'" + loading_stationComboBox.Text + "', S_weigherName=N'" + Properties.Settings.Default.logedUN + "', Weight_Notes=N'" + weight_NotesTextBox.Text + "' where ladingid=" + ladnum;
                    dq.RunNonQuery(upquery);
